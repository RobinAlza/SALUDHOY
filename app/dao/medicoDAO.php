...
class MedicoDAO
{
    ...
    public function consultarPorId()
    {
        return "SELECT id_tipo_identificacion, nombre, apellido, direccion, id_municipio_residencia, fecha_nacimiento, clave, m.id_medico
                FROM persona
                JOIN medico AS m ON persona.numero_identificacion = m.id_numero_identificacion
                WHERE persona.numero_identificacion = $this->numeroIdentificacion";
    }
    ...
    public function agenda($fecha, $id_medico)
    {
        return "SELECT cm.codigo_cita, 
                ...
                JOIN consultorio AS c 
                    ON cm.id_consultorio = c.id_consultorio
                JOIN paciente AS p 
                    ON cm.id_paciente = p.id_paciente
                JOIN tipo_paciente AS tp 
                    ON p.id_tipo_paciente = tp.id_tipo_paciente
                JOIN persona AS per 
                    ON p.id_numero_identificacion = per.numero_identificacion
                WHERE m.id_medico = $id_medico
                AND DATE(cm.fecha_cita) = '{$fecha}'
